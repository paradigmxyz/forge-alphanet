...
abstract contract BaseAuth {
    ...
    /// @notice produce a digest for the authorizer to sign. It enforces that the commit
    ///         depends on the target contract and the calldata, so that the authority
    ///         signature is only valid for those.
    /// @param to - address of the target smart contract
    /// @param data - calldata to send to the target smart contract
    ...
    function getDigest(address to, bytes memory data) public view returns (bytes32 digest) {
    ...
    /// @notice produce a commit that includes the target contract and the calldata for it
    /// @param to - address of the target smart contract
    /// @param data - calldata to send to the target smart contract
    /// @return commit - will be used as part of the digest for the authority to sign.
    function getCommit(address to, bytes memory data) public pure returns (bytes32 commit) {
